...
addEventListener('keydown', e => {
  ...
  if (c === 'KeyG' && window.__toggleRide) window.__toggleRide();
...
(function setupCamera() {
  ...
  cv.addEventListener('pointermove', e => {
    ...
    else cam.dist = Math.min(maxDist(), Math.max(.6, cam.dist + (e.clientY - drag.y) * .002));
  ...
  cv.addEventListener('wheel', e => {
    ...
    else cam.dist = Math.min(maxDist(), Math.max(.6, cam.dist + Math.sign(e.deltaY) * .12));
    e.preventDefault();
  }, { passive: false });
  cv.addEventListener('touchmove', e => {
    if (e.touches.length !== 2 || cam.fps) return;
    const d = Math.hypot(e.touches[0].clientX - e.touches[1].clientX, e.touches[0].clientY - e.touches[1].clientY);
    if (pinch) cam.dist = Math.min(maxDist(), Math.max(.6, cam.dist * (pinch / d)));
    pinch = d; drag = null;
  }, { passive: true });
...
function drawMinimap() {
  ...
  // секции подсвечены своим цветом — на мини-карте сразу видно, где что растёт
  for (const sec of SECTIONS) {
    const r = secRect(sec);
    mmx.fillStyle = 'rgba(255,255,255,.07)';
    mmx.fillRect(r.x0 * MS, r.y0 * MS, (r.x1 - r.x0 + 1) * MS, (r.y1 - r.y0 + 1) * MS);
    mmx.strokeStyle = '#' + sec.c.toString(16).padStart(6, '0');
    mmx.lineWidth = 1;
    mmx.strokeRect(r.x0 * MS + .5, r.y0 * MS + .5, (r.x1 - r.x0 + 1) * MS - 1, (r.y1 - r.y0 + 1) * MS - 1);
  }
  // объекты
...
function updateHUD(dt) {
  ...
  if (window.__rideIcon) window.__rideIcon();
...
function wireUI() {
  ...
  // электрокар: кнопка появляется только когда он куплен
  const rideIcon = () => {
    const b = $('rideBtn');
    b.classList.toggle('hidden', !hasCart());
    b.textContent = G.riding ? '🛑' : '🛺';
    b.title = G.riding ? 'Сойти с кара' : 'Сесть на электрокар';
    b.classList.toggle('on', !!G.riding);
  };
  const toggleRideUI = () => { if (toggleRide()) rideIcon(); };
  $('rideBtn').onclick = toggleRideUI;
  window.__toggleRide = toggleRideUI;
  window.__rideIcon = rideIcon;
  rideIcon();
...
const TUT = [
  ...
  ['Секции 🧭', 'У каждой культуры, каждого животного и каждого вида переработки — <b>своя секция</b> со своим цветом пола и табличкой: помидоры к помидорам, соковыжималки рядом с садом, гриль с фритюрницей. Постройка физически не может встать в чужую секцию, а между секциями аллеи в 3–4 клетки, так что бежать можно не глядя. Заполнил секцию целиком — все её постройки работают на <b>15% быстрее</b>.'],
  ['Цеха 🍳', 'Шесть тематических цехов на 36 мест, погрузочный док с фурами и лента-транспортёр. Принёс мясо к грилю — получил стейк втрое дороже. Цех сам показывает, какого сырья ему не хватает.'],
  ...
  ['Техника 🛞', 'Карта большая, поэтому её надо обживать: <b>❄️ холодильные витрины</b> продлевают срок годности на 60%, <b>🛞 транспортёр</b> сам увозит готовую продукцию из цехов в зал, а <b>🛺 электрокар</b> (кнопка в HUD или <b>G</b>) даёт +45% скорости и +4 к рюкзаку. Всё это во вкладке «Апгрейды».'],
